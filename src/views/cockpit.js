  ...
  return `
    <div class="hero-strip">
      <div class="hero-card confidence">
        ...
        <span class="hero-value">${confPct}%</span>
        <span class="hero-delta"><span class="placeholder"></span> vs 上批次</span>
        <div class="rag-donut-row">
          ${ragDonut(confidence, ragTotal)}
          <div class="rag-legend">
            <span class="rag-legend-item"><span class="rag-dot R"></span>${confidence.red} 红灯</span>
            <span class="rag-legend-item"><span class="rag-dot Y"></span>${confidence.yellow} 黄灯</span>
            <span class="rag-legend-item"><span class="rag-dot G"></span>${confidence.green} 绿灯</span>
          </div>
        </div>
      </div>
      <div class="hero-card">
      ...
      <div class="hero-card">
        ...
        <span class="hero-value">${wave.d30}</span>
        <span class="hero-sub">里程碑交付</span>
      </div>
      <div class="hero-card">
        <span class="hero-label">超负荷人员</span>
        <span class="hero-value"${concentration.overloaded > 0 ? ' style="color: var(--red);"' : ""}>${concentration.overloaded}</span>
        <span class="hero-sub">负荷 ≥ 1.2 / ${totalPeople} 人</span>
      </div>
      <div class="hero-card">
        <span class="hero-label">单点故障项目</span>
        <span class="hero-value"${concentration.bf1Count > 0 ? ' style="color: var(--red);"' : ""}>${concentration.bf1Count}</span>
    ...
    <div class="panel-row">
      <div class="panel">
        ...
        ${actNow.length ? `<table class="risk-table">
          ...
          <tbody>${actNow.map(r => `<tr class="clickable" data-open-project="${escapeHtml(r.projectId)}">
              <td><span class="rag-tag ${r.rag}"></span><span class="project-name">${escapeHtml(r.projectName)}</span><br><span class="owner">${escapeHtml(r.owner)} · ${escapeHtml(r.dept)}</span></td>
              <td><span class="milestone-name">${escapeHtml(r.milestoneName)}</span></td>
              <td class="text-right slip-days">${r.slipDays}d</td>
            </tr>`).join("")}</tbody>
        </table>` : '<p class="muted">当前筛选范围内暂无风险项目。</p>'}
      </div>
      ...
      <div class="panel">
        ...
        <div class="trend-stub">周趋势数据将在下次上传后可用</div>
        <div style="margin-top: 8px;">
          <div class="panel-header"><span class="panel-title">项目阶段分布</span></div>
          <div style="display: grid; grid-template-columns: repeat(${phases.length}, 1fr); gap: 8px; margin-top: 12px;">
            ${phases.map(ph => `<div class="conc-stat"><div class="conc-stat-value" style="font-size: 20px;">${ph.count}</div><div class="conc-stat-label">${escapeHtml(ph.label)}</div></div>`).join("")}
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="panel-row">
      <div class="panel">
        ...
        <div class="conc-stats">
          <div class="conc-stat"><div class="conc-stat-value${concentration.bf1Count > 0 ? " danger" : ""}">${concentration.bf1Count}</div><div class="conc-stat-label">BF=1 项目<br>(单点故障)</div></div>
          <div class="conc-stat"><div class="conc-stat-value${concentration.overAllocated > 0 ? " warning" : ""}">${concentration.overAllocated}</div><div class="conc-stat-label">超分配人员<br>(投入 &gt; 100%)</div></div>
          <div class="conc-stat"><div class="conc-stat-value${concentration.overloaded > 0 ? " danger" : ""}">${concentration.overloaded}</div><div class="conc-stat-label">超负荷人员<br>(负荷 ≥ 1.2)</div></div>
        </div>
        <div style="margin-top: 4px;">
          <div class="panel-header"><span class="panel-title" style="font-size: 12px; color: var(--muted);">高风险关键人员</span></div>
          <div class="key-person-list">
            ${concentration.keyPersons.length ? concentration.keyPersons.map(kp => `<div class="key-person-row">
              <span><span class="key-person-name">${escapeHtml(kp.person)}</span> <span class="key-person-role">· ${escapeHtml(kp.role)}</span></span>
              <span style="font-size: 12px; color: var(--muted);">投入 ${Math.round(kp.ratio * 100)}%</span>
              <span class="key-person-load ${kp.load >= 1.2 ? "load-high" : kp.load >= 0.6 ? "load-med" : ""}">${kp.load.toFixed(2)}</span>
            </div>`).join("") : '<p class="muted" style="padding: 8px 0;">暂无高风险关键人员。</p>'}
          </div>
        </div>
      </div>
      ...
      <div class="panel">
        ...
        </div>` : '<p class="muted">暂无业务域健康数据。</p>'}
        <div class="trend-stub">组织级趋势将在下次批次比对后可用</div>
        <div style="margin-top: 4px;">
          <div class="panel-header"><span class="panel-title">人力利用率</span></div>
          <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 8px; margin-top: 12px;">
            <div class="conc-stat"><div class="conc-stat-value" style="font-size: 22px; color: var(--green);">${workforce.low}</div><div class="conc-stat-label">正常负荷<br>(低于 0.6)</div></div>
            <div class="conc-stat"><div class="conc-stat-value" style="font-size: 22px; color: var(--yellow);">${workforce.mid}</div><div class="conc-stat-label">中等负荷<br>(0.6 – 1.2)</div></div>
            <div class="conc-stat"><div class="conc-stat-value" style="font-size: 22px; color: var(--red);">${workforce.high}</div><div class="conc-stat-label">超负荷<br>(≥ 1.2)</div></div>
          </div>
        </div>
      </div>
    </div>`;
...
function ragDonut(confidence, total) {
  ...
  return `<svg class="rag-donut" viewBox="0 0 36 36">
    <circle cx="18" cy="18" r="14" fill="none" stroke="var(--gray-bg)" stroke-width="5"></circle>
    ${redPct > 0 ? `<circle cx="18" cy="18" r="14" fill="none" stroke="var(--red)" stroke-width="5" stroke-dasharray="${redPct} ${100 - redPct}" stroke-dashoffset="25" transform="rotate(-90 18 18)"></circle>` : ""}
    ${greenPct > 0 ? `<circle cx="18" cy="18" r="14" fill="none" stroke="var(--green)" stroke-width="5" stroke-dasharray="${greenPct} ${100 - greenPct}" stroke-dashoffset="${25 - redPct - yellowPct}" transform="rotate(-90 18 18)"></circle>` : ""}
  </svg>`;
}
